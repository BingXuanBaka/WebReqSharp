    ...
    public class Request
    {
        ...
        private byte[] GetFormDataBytes(string boundary,List<TextData> textDatas,List<ByteData> fileDatas)
        {
            ...
            foreach (TextData textData in textDatas)
            {
                ...
                memStream.Write(GetTextHeader(this.boundary, textData.Name));
                memStream.Write(Encoding.UTF8.GetBytes(textData.Context + "\r\n"));
                break;
            }
            ...
            foreach (ByteData fileData in fileDatas)
            {
                ...
                memStream.Write(GetFileHeader(this.boundary, fileData.Name,fileData.FileName,fileData.ContentType));
                memStream.Write(fileData.Bytes);
